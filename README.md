...
For ordinary users, you only have to remember one macro -- ``@devec``. Putting it before the assignments that you want to *de-vectorize*, it will automatically translate your expressions into efficient loops. For example, you can write

```julia
@devec r = exp(a + b) .* sum(c)
```

To inspect the code generated by *DeExpr*, you can write
